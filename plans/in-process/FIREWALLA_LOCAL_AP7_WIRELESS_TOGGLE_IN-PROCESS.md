...
- [ ] **Blocked on reporter (re-capture):** Ask the reporter to (1) hit **"Sync Runtime"**, wait a few seconds, (2) download the diagnostic, (3) make the wireless change in the app, (4) hit **"Sync Runtime"** again, wait, (5) download a second diagnostic, and post both. The new `runtime_init_payload` field will expose the raw wireless config for comparison.
